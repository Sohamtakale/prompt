...
export default function TimelinePage() {
  ...
  function toggle(stage: TimelineStage) {
  ...
  return (
    <div className="max-w-[120rem] mx-auto px-6 lg:px-12 py-12">
            ...
            >
              <div className={`w-1/2 flex ${index % 2 === 0 ? 'justify-end pr-8 md:pr-12 lg:pr-16' : 'justify-start pl-8 md:pl-12 lg:pl-16'}`}>
                <button
                  ...
                  className={`glass-card text-left w-full cursor-pointer transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${
                    selectedStage?.id === stage.id ? 'ring-2 ring-accent ring-offset-2 ring-offset-surface scale-[1.02]' : 'hover:scale-[1.02]'
                  }`}
                  aria-label={`${stage.stage} — tap to ${selectedStage?.id === stage.id ? 'collapse' : 'expand'}`}
                ...
                >
                  ...
                  {selectedStage?.id === stage.id && (
                    <div
                      ...
                      className="mt-8 pt-8 border-t border-white/10 animate-fade-in text-left space-y-6"
                    >
              ...
              <div className="absolute left-1/2 -translate-x-1/2 flex items-center justify-center w-12 h-12 z-10 flex-shrink-0" aria-hidden="true">
      ...
      <div className="md:hidden">
        <ol className="flex overflow-x-auto gap-6 pb-8 pt-4 snap-x snap-mandatory -mx-4 px-4 no-scrollbar">
          {stages.map((stage, index) => (
            <li key={stage.id} className="w-[340px] snap-center flex-shrink-0">
              <button
                type="button"
                onClick={() => toggle(stage)}
                style={{ padding: '2rem' }}
                className={`glass-card w-full cursor-pointer text-center transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent ${
                  selectedStage?.id === stage.id ? 'ring-2 ring-accent scale-105' : 'scale-100'
                }`}
                aria-label={`${stage.stage} — tap to ${selectedStage?.id === stage.id ? 'collapse' : 'expand'}`}
                aria-expanded={selectedStage?.id === stage.id}
                aria-controls={`stage-detail-mobile-${stage.id}`}
              >
                <div className="flex flex-col items-center justify-center mb-6 gap-3">
                  <span className="w-12 h-12 rounded-full bg-accent/20 text-accent-light shadow-[0_0_15px_rgba(129,140,248,0.2)] flex items-center justify-center text-xl font-black" aria-hidden="true">
                    {index + 1}
                  </span>
                ...
                {selectedStage?.id === stage.id && (
                  <div
                    id={`stage-detail-mobile-${stage.id}`}
                    className="mt-6 pt-6 border-t border-white/10 animate-fade-in space-y-4"
                  >
